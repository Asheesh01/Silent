  ...
  return (
    <div className="bg-white h-[52px] flex justify-between py-1 xl:px-[208px] fixed top-0 w-[100%] z-[1000]">
      ...
      <div className="flex gap-2 items-center">
        ...
        <div className="relative">
          <input
            ...
            className="searchInput w-[300px] bg-gray-100 rounded-sm h-[35px] px-4"
            type="text"
            placeholder="Search"
          />
          {searchUser.length > 0 && debouncedTerm.length !== 0 && (
            <div className="absolute w-[calc(100vw-2rem)] sm:w-[352px] bg-gray-200 rounded-2xl">
              {searchUser.map((item) => (
                <Link key={item._id} to={`/profile/${item._id}`} className="flex gap-2 items-center cursor-pointer" onClick={() => setSearchTerm("")}>
                  <div><img className="w-[40px] h-[40px] rounded-full" src={item?.profile_pic} alt="" /></div>
                  <div>{item?.f_name}</div>
                </Link>
              ))}
            </div>
      ...
      <Link to={'/message'} className="flex flex-col items-center cursor-pointer">
        ...
        <div className={`text-sm text-gray-500 ${location.pathname === '/message' ? "border-b-3" : ""}`}>Message</div>
      </Link>
